PLACEHOLDER_ID = 90100

# replace water at destroyed custom block with air (water origins from custom block being based on a waterlogged block)
execute as @e[type=minecraft:item,nbt={Item:{components:{"minecraft:custom_model_data": PLACEHOLDER_ID}}}] run 
...
execute as @a store result score @s heldItem run
    data get entity @s SelectedItem.components."minecraft:custom_model_data"
